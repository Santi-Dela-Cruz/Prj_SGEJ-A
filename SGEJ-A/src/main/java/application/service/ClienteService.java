...
public class ClienteService {
    ...
    public ResultadoOperacion registrarCliente(Cliente cliente) {
        ...
        if (clienteDAO.existeCliente(cliente.getNumeroIdentificacion())) {
            return new ResultadoOperacion(false, "Ya existe un cliente con el número de identificación: " + cliente.getNumeroIdentificacion());
        }
    ...
    private ResultadoOperacion validarCliente(Cliente cliente) {
        ...
        if (cliente.getTipoPersona() == Cliente.TipoPersona.NATURAL) {
            if (cliente.getEstadoCivil() == null || cliente.getEstadoCivil().trim().isEmpty()) {
                return new ResultadoOperacion(false, "El estado civil es obligatorio para personas naturales");
            }
        } else if (cliente.getTipoPersona() == Cliente.TipoPersona.JURIDICA) {
    ...
    private ResultadoOperacion validarDatosActualizables(Cliente cliente) {
        ...
        if (cliente.getTipoPersona() == Cliente.TipoPersona.NATURAL) {
            if (cliente.getEstadoCivil() != null && cliente.getEstadoCivil().trim().isEmpty()) {
                return new ResultadoOperacion(false, "El estado civil no puede estar vacío para personas naturales");
            }
        } else if (cliente.getTipoPersona() == Cliente.TipoPersona.JURIDICA) {
            if (cliente.getRepresentanteLegal() != null && cliente.getRepresentanteLegal().trim().isEmpty()) {
                return new ResultadoOperacion(false, "El representante legal no puede estar vacío para personas jurídicas");
            }
            if (cliente.getDireccionFiscal() != null && cliente.getDireccionFiscal().trim().isEmpty()) {
                return new ResultadoOperacion(false, "La dirección fiscal no puede estar vacía para personas jurídicas");
            }
        }
